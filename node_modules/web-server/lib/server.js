...
const uploadImagesConfig = require('../config/file-upload-images');
...
function Server() {
  ...
  // mapping server
  this.mappingServer = function (mappings) {
      ...
      if (!isEmpty(mappings) && isArray(mappings)) {
        mappings.map(mapping => {
          ...
          switch (method) {
            ...
            case 'UPLOAD':
              return app.use(pathServer, routerMappings.put(pathName, uploadImagesConfig.array('images', 10), serviceName[methodName]));
            default:
              return null;
          }
        })
  ...
  this.repository = function () {
    ...
    const name = process.env.NAME || 'accLq';
    const db_url = `${database}://${host}:${port}/${name}`;
    return new Promise((resolve, reject) => {

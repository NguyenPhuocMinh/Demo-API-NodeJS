    ...
    return new Promise((resolve, reject) => {
      ...
      if (!isEmpty(mappings) && isArray(mappings)) {
        mappings.map(mapping => {
          ...
          switch (method) {
            case 'GET':
              return app.use(routerMappings.get(pathName, serviceName[methodName]));
            case 'POST':
              return app.use(routerMappings.post(pathName, serviceName[methodName]));
            case 'GET_ID':
              return app.use(routerMappings.get(pathName, serviceName[methodName]));
            case 'PUT':
              return app.use(routerMappings.put(pathName, serviceName[methodName]));
            case 'UPLOAD':
              return app.use(routerMappings.put(pathName, uploadImagesConfig.array('images', 10), serviceName[methodName]));
            default:
              return null;
          }
        })

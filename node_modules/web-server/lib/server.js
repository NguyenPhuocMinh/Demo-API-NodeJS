...
const loggingFactory = require('../config/logger');
const moment = require('moment');
const momentTimezone = require('moment-timezone');
const slugifyString = require('../config/utils/slugifyString');
const { mongodb_url, mongodb_local_url } = require('../config/mongo_url');
const { uploadImages } = require('web-upload');
...
function Server() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!isEmpty(mappings) && isArray(mappings)) {
        mappings.map(mapping => {
          ...
          const transFormInput = get(mapping, 'transFormInput');
          console.log("this.mappingApi -> transFormInput", transFormInput)
          switch (method) {
            case 'GET':
              return app.use(pathServer, routerMappings.get(pathName, serviceName[methodName]));
            case 'POST':
              return app.use(pathServer, routerMappings.post(pathName, serviceName[methodName]));
            case 'GET_ID':
              return app.use(pathServer, routerMappings.get(pathName, serviceName[methodName]));
            case 'PUT':
              return app.use(pathServer, routerMappings.put(pathName, serviceName[methodName]));
            case 'DELETE':
              return app.use(pathServer, routerMappings.delete(pathName, serviceName[methodName]));
            default:
              return null;
          }
        })
  ...
  // connect database
  this.repository = function (sandboxConfig) {
    const isMongodbLocal = get(sandboxConfig, 'application.isMongodbLocal');
    return new Promise((resolve, reject) => {
      let db_url = process.env.DB_URL || mongodb_url;
      if (isMongodbLocal) {
        db_url = mongodb_local_url;
      }
